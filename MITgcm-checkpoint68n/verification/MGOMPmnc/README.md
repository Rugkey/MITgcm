...
#!/usr/bin/env python3
"""
生成污染物排放文件用于MITgcm POLLUTANT包
基于全球河流排放数据(river_emissions.csv)
"""
...
def create_pollutant_emission_file(filename='pollutant_emission.bin', 
    ...
    - nx, ny: 网格大小
    - nrec: 记录数
    - use_ocean_mask: 是否使用海洋掩码
    """
    
    print(f"正在创建污染物排放文件: {filename}")
    print(f"网格大小: {nx} x {ny}")
    print(f"记录数: {nrec}")
    
    # 读取河流排放数据
    csv_file = '../river_emissions.csv'
    if not os.path.exists(csv_file):
        print(f"错误: 找不到文件 {csv_file}")
        return False
    
    print(f"读取河流排放数据: {csv_file}")
    rivers_df = pd.read_csv(csv_file)
    print(f"读取到 {len(rivers_df)} 条河流数据")
    ...
    # 将河流排放分配到网格
    rivers_added = 0
    total_emission_before_mask = 0
    
    # 显示前几条河流的分配情况
    print("前5条河流的分配情况:")
    for idx, (_, river) in enumerate(rivers_df.iterrows()):
        # 计算网格索引 (lon: -180到180, lat: -90到90)
        i = int((river['lon'] + 180) * nx / 360)
        j = int((river['lat'] + 90) * ny / 180)
        
        # 确保索引在范围内
        if 0 <= i < nx and 0 <= j < ny:
            # 使用Mid列数据，转换为mol/m^2/s
            # Mid列单位假设为kg/s，需要转换为mol/m^2/s
            # 这里使用简单的转换因子1e-6
            emission_value = river['Mid'] * 1e-6
            emission_data[j, i] += emission_value
            rivers_added += 1
            total_emission_before_mask += emission_value
            
            # 显示前5条河流的详细信息
            if idx < 5:
                print(f"  河流 {river['rivers']}: 位置({river['lat']:.1f}, {river['lon']:.1f}) -> 网格({j}, {i}), 排放值: {emission_value:.2e}")
    
    print(f"成功添加 {rivers_added} 条河流到网格")
    print(f"应用掩码前总排放量: {total_emission_before_mask:.2e} mol/s")
    
    # 如果使用海洋掩码，只保留海洋网格点
    if use_ocean_mask:
        mask_file = './ETOPO/topo.bin'
        if os.path.exists(mask_file):
            print(f"应用海洋掩码: {mask_file}")
            # 读取地形文件 (假设是单精度浮点数)
            with open(mask_file, 'rb') as f:
                mask_data = np.frombuffer(f.read(), dtype='>f4')
                mask_data = mask_data.reshape((ny, nx))
            
            print(f"地形数据范围: {mask_data.min():.2f} 到 {mask_data.max():.2f}")
            
            # 海洋网格点 (深度 < 0) - 修正逻辑
            ocean_mask = mask_data < 0
            print(f"海洋网格点数: {np.sum(ocean_mask)}")
            
            # 应用掩码前先保存原始数据用于调试
            print(f"应用掩码前非零排放点数: {np.sum(emission_data > 0)}")
            emission_data = emission_data * ocean_mask
            print(f"应用掩码后非零排放点数: {np.sum(emission_data > 0)}")
        else:
            print(f"警告: 找不到海洋掩码文件 {mask_file}")
    ...
    with open(filename, 'wb') as f:
        for rec in range(nrec):
            ...
            for j in range(ny):
                for i in range(nx):
                    # 使用单精度浮点数 (4字节)，big-endian格式
                    f.write(struct.pack('>f', emission_data[j, i]))
    
    # 统计信息
    max_emission = np.max(emission_data)
    non_zero_emissions = emission_data[emission_data > 0]
    mean_emission = np.mean(non_zero_emissions) if len(non_zero_emissions) > 0 else 0.0
    total_emission = np.sum(emission_data)
    
    print(f"\n排放文件创建完成!")
    print(f"文件大小: {os.path.getsize(filename)} 字节")
    print(f"最大排放率: {max_emission:.2e} mol/m^2/s")
    print(f"平均排放率: {mean_emission:.2e} mol/m^2/s")
    print(f"总排放量: {total_emission:.2e} mol/s")
    print(f"非零网格点数: {np.sum(emission_data > 0)}")
    
    return True

if __name__ == "__main__":
    # 创建排放文件
    success = create_pollutant_emission_file()
