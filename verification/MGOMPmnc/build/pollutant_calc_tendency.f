...
C     pollutant_monFreq         :: frequency for pollutant monitor (s)
     ...
     &              pollutant_monFreq
      Real*8 pollutant_decay_rate
      Real*8 pollutant_settle_rate
      Real*8 pollutant_biouptake_rate
      Real*8 pollutant_forcingPeriod
      Real*8 pollutant_forcingCycle
      Real*8 pollutant_monFreq
...
C     pollutant_mass    :: pollutant mass per grid cell (mol)
     ...
     &              pollutant_flux, pollutant_conc, pollutant_mass
      Real*8 pollutant_flux(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 pollutant_conc(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8 pollutant_mass(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
...
C     myThid        :: my Thread Id number
      Real*8  pTr_Pollutant((1-OLx):(sNx+OLx),(1-OLy):(sNy+OLy),Nr)
      Real*8  gPollutant((1-OLx):(sNx+OLx),(1-OLy):(sNy+OLy),Nr)
      INTEGER bi, bj
...
C     !LOCAL VARIABLES:
      INTEGER i, j, k
      Real*8 sourceTerm, sinkTerm
      Real*8 sourceDiag((1-OLx):(sNx+OLx),(1-OLy):(sNy+OLy),Nr)
      Real*8 sinkDiag((1-OLx):(sNx+OLx),(1-OLy):(sNy+OLy),Nr)
      Real*8 decayDiag((1-OLx):(sNx+OLx),(1-OLy):(sNy+OLy),Nr)
      Real*8 settleDiag((1-OLx):(sNx+OLx),(1-OLy):(sNy+OLy),Nr)
      Real*8 bioupDiag((1-OLx):(sNx+OLx),(1-OLy):(sNy+OLy),Nr)
      Real*8 totalDiag((1-OLx):(sNx+OLx),(1-OLy):(sNy+OLy),Nr)
      Real*8 pTr_Pollutant_safe
CEOP
...
C     Initialize diagnostic arrays
      DO k = 1, Nr
       DO j = (1-OLy), (sNy+OLy)
        DO i = (1-OLx), (sNx+OLx)
         sourceDiag(i,j,k) = 0.D0
         sinkDiag(i,j,k) = 0.D0
         decayDiag(i,j,k) = 0.D0
         settleDiag(i,j,k) = 0.D0
         bioupDiag(i,j,k) = 0.D0
         totalDiag(i,j,k) = 0.D0
        ENDDO
       ENDDO
      ENDDO
...
C     Apply concentration protection to local variable (don't modify tracer)
          pTr_Pollutant_safe = MAX( 0.D0, pTr_Pollutant(i,j,k) )
         
C     Source term (only for surface layer)
          IF ( k.EQ.1 ) THEN
           sourceTerm = pollutant_flux(i,j,bi,bj) / drF(1)
          ELSE
           sourceTerm = 0.D0
          ENDIF
...
C     Sink terms
          ...
          IF ( usePollutantDecay ) THEN
           ...
           IF ( pTr_Pollutant_safe .GT. 0.D0 ) THEN
     ...
     &                        * pTr_Pollutant_safe
          ...
          IF ( usePollutantSettling ) THEN
           sinkTerm = sinkTerm + pollutant_settle_rate
           IF ( pTr_Pollutant_safe .GT. 0.D0 ) THEN
            settleDiag(i,j,k) = pollutant_settle_rate 
     &                        * pTr_Pollutant_safe
           ELSE
            settleDiag(i,j,k) = 0.D0
           ENDIF
...
C     Apply source and sink terms to tracer tendency
C     Only apply sink terms if concentration is positive
...
C     Calculate total tendency for diagnostics
          totalDiag(i,j,k) = sourceTerm - sinkDiag(i,j,k)
         
C     Final protection: ensure tendency won't make tracer negative
C     This is the key protection mechanism - limit tendency to prevent negative values
          IF ( pTr_Pollutant(i,j,k) + gPollutant(i,j,k) .LT. 0.D0 ) THEN
