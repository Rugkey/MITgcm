...
c     PARAMETER( GCHEM_tendTr_num = 3 )
      ...
      PARAMETER( GCHEM_tendTr_num = CFC_Tr_num + SPOIL_Tr_num + 1 )
CEOP
       ...
       DO bj=myByLo(myThid),myByHi(myThid)
        DO bi=myBxLo(myThid),myBxHi(myThid)
         DO k = 1, Nr
          DO j = 1-OLy, sNy+OLy
           DO i = 1-OLx, sNx+OLx
            gchemTendency(i,j,k,bi,bj,jTr) = 0.D0
           ENDDO
          ENDDO
         ENDDO
        ENDDO
...
C        are corrected before being passed to the source-sink calculation.
         DO k = 1, Nr
          DO j = jMin, jMax
           DO i = iMin, iMax
            pTracer(i,j,k,bi,bj,iTr) = MAX(0.D0,
