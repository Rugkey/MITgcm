...
C     pollutantFlux1      :: temporary array for time interpolation
     ...
     &    pollutantFlux0, pollutantFlux1
      ...
      Real*8 pollutantFlux0(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 pollutantFlux1(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
       ...
       IF ( myIter .EQ. nIter0 ) THEN
         DO bj = myByLo(myThid), myByHi(myThid)
          DO bi = myBxLo(myThid), myBxHi(myThid)
            ...
            DO j=1-OLy,sNy+OLy
             DO i=1-OLx,sNx+OLx
              pollutantFlux0(i,j,bi,bj) = 0.D0
              pollutantFlux1(i,j,bi,bj) = 0.D0
              pollutant_flux(i,j,bi,bj) = 0.D0
             ENDDO
            ENDDO
          ENDDO
         ENDDO
...
C--   If the above condition is met then we need to read in new data.
     ...
     &          intime1, myIter, myThid)
         ...
         CALL EXCH_XY_RS ( pollutantFlux0,  myThid  )
         CALL EXCH_XY_RS ( pollutantFlux1,  myThid  )
...
C--   Sanity Check: Calculate and print total global emission on first time step
     ...
     &                   + pollutantFlux0(i,j,bi,bj) * rA(i,j,bi,bj)
          ...
          CALL GLOBAL_SUM_R8 (  local_total_emission,  myThid  )
     ...
     &            pollutant_flux(i,j,bi,bj) / wetArea
       ...
       CALL EXCH_XY_RS ( pollutant_flux,  myThid  )
