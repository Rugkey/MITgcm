...
C--   Set default values
       ...
       pollutant_Tc              = -100.D0
       pollutant_k_dark_20_d   = 0.D0
       pollutant_Q10_dark      = 2.0D0
       pollutant_k_bio_20_d    = 0.D0
       pollutant_Q10_bio       = 2.0D0
       pollutant_k_OH_d        = 0.D0
       pollutant_k_photo_d     = 0.D0
       pollutant_light_atten   = 0.D0
...
C---+----1----+----2----+----3----+----4----+----5----+----6----+----7-|--+----|
      ...
      IF ( pollutant_k_dark_20_d .LT. 0.D0 ) THEN
     ...
     &    'pollutant_k_dark_20_d must be >= 0'
      ...
      IF ( pollutant_k_bio_20_d .LT. 0.D0 ) THEN
        WRITE(msgBuf,'(2A)') 'POLLUTANT_READPARMS: ',
     &    'pollutant_k_bio_20_d must be >= 0'
      ...
      IF ( pollutant_k_OH_d .LT. 0.D0 ) THEN
        WRITE(msgBuf,'(2A)') 'POLLUTANT_READPARMS: ',
     &    'pollutant_k_OH_d must be >= 0'
      ...
      IF ( pollutant_k_photo_d .LT. 0.D0 ) THEN
        WRITE(msgBuf,'(2A)') 'POLLUTANT_READPARMS: ',
     &    'pollutant_k_photo_d must be >= 0'
      ...
      IF ( pollutant_light_atten .LT. 0.D0 ) THEN
        WRITE(msgBuf,'(2A)') 'POLLUTANT_READPARMS: ',
